				 ...
				 struct gve_tx_alloc_rings_cfg *cfg)
{
	...
	kfree(tx->dqo.xsk_reorder_queue);
	...
	if (idx >= cfg->qcfg->num_queues && cfg->num_xdp_rings) {
		...
		tx->dqo.xsk_reorder_queue =
			kvcalloc(tx->dqo.complq_mask + 1,
				 ...
				 GFP_KERNEL);
#else /* LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0) */
		tx->dqo.xsk_reorder_queue = kcalloc(tx->dqo.complq_mask + 1,
						    sizeof(tx->dqo.xsk_reorder_queue[0]),
						    GFP_KERNEL);
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0) */
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6,2,1) && LINUX_VERSION_CODE < KERNEL_VERSION(7,0,0) && !RHEL_VERSION_GTE(10,3))
	if (skb_is_gso(skb) && unlikely(ipv6_hopopt_jumbo_remove(skb))) goto drop;
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(6,2,1) && LINUX_VERSION_CODE < KERNEL_VERSION(7,0,0) && !RHEL_VERSION_GTE(10,3)) */
	if (tx->dqo.qpl) {
